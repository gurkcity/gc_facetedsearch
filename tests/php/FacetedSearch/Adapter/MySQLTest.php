...
namespace Onlineshopmodule\PrestaShop\Module\FacetedSearch\Tests\Adapter;
...
class MySQLTest extends MockeryTestCase
{
    ...
    protected function setUp()
    {
        ...
        $configurationMock->shouldReceive('get')
            ->with('GC_LAYERED_FILTER_SHOW_OUT_OF_STOCK_LAST')
            ->andReturn(0);
    ...
    public function testGetMinMaxPriceValue()
    {
        ...
        $dbInstanceMock->shouldReceive('executeS')
            ->once()
            ->with('SELECT psi.price_min, MIN(price_min) as min, MAX(price_max) as max FROM ps_product p INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3)')
            ->andReturn(
                [
                    [
                        'price_min' => '11',
    ...
    public function testGetQueryWithManyFilters()
    {
        ...
        $this->assertEquals(
            'SELECT p.id_product, sa.out_of_stock, SUM(sa.quantity) as quantity, psi.price_min, psi.price_max, psi.range_start, psi.range_end FROM ps_product p LEFT JOIN ps_product_attribute pa ON (p.id_product = pa.id_product) LEFT JOIN ps_product_attribute_combination pac ON (pa.id_product_attribute = pac.id_product_attribute) LEFT JOIN ps_stock_available sa ON (p.id_product = sa.id_product AND IFNULL(pac.id_product_attribute, 0) = sa.id_product_attribute) INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) WHERE p.condition IN (\'new\', \'used\') AND p.weight=\'10\' AND psi.price_min>=10 AND psi.price_min<=100 AND p.id_product IN (2, 20, 200) ORDER BY p.id_product DESC',
            $this->adapter->getQuery()
        );
    }
    ...
    public function getManyOperationsFilters()
    {
        return [
            [
                ...
                'expected' => 'SELECT p.id_product, sa.out_of_stock, SUM(sa.quantity) as quantity, psi.price_min, psi.price_max, psi.range_start, psi.range_end FROM ps_product p LEFT JOIN ps_product_attribute pa ON (p.id_product = pa.id_product) LEFT JOIN ps_product_attribute_combination pac ON (pa.id_product_attribute = pac.id_product_attribute) LEFT JOIN ps_stock_available sa ON (p.id_product = sa.id_product AND IFNULL(pac.id_product_attribute, 0) = sa.id_product_attribute) INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) LEFT JOIN ps_stock_available sa_1 ON (p.id_product = sa_1.id_product AND IFNULL(pac.id_product_attribute, 0) = sa_1.id_product_attribute) WHERE ((sa.quantity>=0 AND sa_1.out_of_stock IN (1, 3, 4)) OR (sa.quantity>0 AND sa_1.out_of_stock=1)) ORDER BY p.id_product DESC',
            ],
            [
                'fields' => [
                    'id_product',
    ...
    public function oneSelectFieldDataProvider()
    {
        return [
            ...
            ['price_min', 'SELECT psi.price_min FROM ps_product p INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) ORDER BY p.id_product DESC'],
            ['price_max', 'SELECT psi.price_max FROM ps_product p INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) ORDER BY p.id_product DESC'],
            ['range_start', 'SELECT psi.range_start FROM ps_product p INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) ORDER BY p.id_product DESC'],
            ['range_end', 'SELECT psi.range_end FROM ps_product p INNER JOIN ps_gc_facetedsearch_price_index psi ON (psi.id_product = p.id_product AND psi.id_shop = 1 AND psi.id_currency = 4 AND psi.id_country = 3) ORDER BY p.id_product DESC'],
