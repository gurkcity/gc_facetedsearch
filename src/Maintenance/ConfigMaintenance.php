...
{
    public const VALUE_STRIP_LENGTH = 200;
    ...
    public function get(): array
    {
        ...
        foreach ($this->config as $config) {
            ...
            $configMaintenance[$configName] = [
                ...
                'constraint' => $this->getConfigurationConstraint($config),
            ];
        }
    ...
    public function getConfigurationConstraint(Config $config): string
    {
        $idShop = (int) $this->shopContext->getContextShopID();
        $idShopGroup = (int) $this->shopContext->getContextShopGroup()->id;

        if ($idShop) {
            $shopConstraint = ShopConstraint::shop($idShop, true);
        ...
        if ($idShopGroup) {
            $shopConstraint = ShopConstraint::shopGroup($idShopGroup, true);
