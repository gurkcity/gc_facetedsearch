...
trait ModuleFunctionsTrait
{
    ...
    public function indexProductPrices(int $idProduct, $smart = true)
    {
        ...
        foreach ($shopList as $idShop) {
            ...
            if (!$this->getConfig()->get('FILTER_PRICE_USETAX')) {
                $taxRatesByCountry = [];
            }
            ...
            if ($omitCountries) {
                ...
                if (!in_array($defaultCountryId, $indexedCountryIds)) {
                    $taxRatesByCountry[] = [
                        'rate' => 0,
                        'id_country' => $defaultCountryId,
                        'iso_code' => Country::getIsoById($defaultCountryId),
                    ];
                }
            } else {
    ...
    private function migratePsConfiguration()
    {
        $configMap = [
            ...
            'PS_LAYERED_FILTER_FEATURE_VALUES_USE_POSITION' => 'GC_FACETEDSEARCH_FILTER_FEATURE_VALUES_USE_POSITION',
