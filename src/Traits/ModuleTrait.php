...
use Onlineshopmodule\PrestaShop\Module\Facetedsearch\Log\Logger;
...
use Onlineshopmodule\PrestaShop\Module\Facetedsearch\Mail\MailPartialTemplateRenderer;
...
use Onlineshopmodule\PrestaShop\Module\Facetedsearch\Module\Install;
use Onlineshopmodule\PrestaShop\Module\Facetedsearch\Settings;
...
trait ModuleTrait
{
     ...
     * const GC_VERSION = '9.0.9';
     * const GC_SUBVERSION = '58';
     */
    protected $GC_VERSION = '9.0.9';
    protected $GC_SUBVERSION = '70';
    ...
    public $sendMailOrderStatusAwaitingPayment = true;
    public $redirectAdminConfigurationPermanentTo = '';
    public $logger;
    ...
    public function initModule()
    {
        ...
        if ($this->isPayment()) {
            $this->templatesForEmailVars[] = $this->name . '_payment';
        }
    ...
    public function hookActionAdminControllerSetMedia(array $params)
    {
        ...
        if (strpos($route, $this->name) !== false) {
            ...
            }, $this->getSettings()->getTabs());

            \Media::addJsDef([
                'txtUpdateLicenseCode' => $this->trans('Do you really want to update the license code for this module?', [], 'Modules.Gcfacetedsearch.Admin'),
    ...
    public function hookActionFrontControllerSetMedia(array $params)
    {
        ...
        $this->registerStylesheet('views/css/front/module.css', $cssParams);
        $this->registerJavascript('views/js/front/module.js', $jsParams);

        if (
            $this->isPayment()
            && $this->context->controller instanceof \OrderControllerCore
        ) {
            $this->registerPaymentScript($context->controller);
        }
    ...
    public function getOrderExtraMailVars(\Order $order)
    {
            ...
            if (isset($customizedDatas[$product['product_id']][$product['product_attribute_id']])) {
                ...
                foreach ($customizedDatas[$product['product_id']][$product['product_attribute_id']][$order->id_address_delivery] as $customization) {
                    ...
                    if (isset($customization['datas'][\Product::CUSTOMIZE_FILE])) {
                        $customizationText .= $this->trans('%count% image(s)', ['%count%' => count($customization['datas'][\Product::CUSTOMIZE_FILE])], 'Modules.Gcfacetedsearch.Admin') . '<br />';
                    }
        ...
        $data = [
            ...
            '{carrier}' => ($order->isVirtual() || $carrier->name == '') ? $this->trans('No carrier', [], 'Modules.Gcfacetedsearch.Admin') : $carrier->name,
            '{payment}' => \Tools::substr($order->payment, 0, 255) . ($order->hasBeenPaid() ? '' : '&nbsp;' . $this->trans('(waiting for validation)', [], 'Modules.Gcfacetedsearch.Admin')),
            ...
            '{recycled_packaging_label}' => $order->recyclable ? $this->trans('Yes', [], 'Modules.Gcfacetedsearch.Admin') : $this->trans('No', [], 'Modules.Gcfacetedsearch.Admin'),
        ];
    ...
    public function getConfig(): ConfigurationAdapter
    {
        if (empty($this->config)) {
            try {
                $this->config = $this->get('onlineshopmodule.module.facetedsearch.configurationadapter');
            } catch (\Throwable $e) {
                // Do nothing!
            }
    ...
    public function getLogger(): Logger
    {
        if (empty($this->logger)) {
            try {
                $this->logger = $this->get('onlineshopmodule.module.facetedsearch.logger');
            } catch (\Throwable $e) {
                // Do nothing!
            }
    ...
    public function registerPaymentScript(\FrontController $controller): void
    {
        $jsParams = [
            'position' => 'bottom',
            'priority' => 2001,
        ...
        if ($this->getConfig()->get('JS_DEFER')) {
            $jsParams['attributes'] = 'defer';
        }
        ...
        $this->registerStylesheet('views/css/front/payment.css', $cssParams);
        $this->registerJavascript('views/js/front/payment.js', $jsParams);
    }
    ...
    protected function registerJavascript($file = 'views/js/front/module.js', $jsParams = [])
    {
        if ($this->getConfig()->get('JS_DEFER')) {
            $jsParams['attributes'] = 'defer';
        }
        ...
        if ($this->getConfig()->get('THEME') === 'hummingbird') {
            $fileHummingbird = str_replace('.js', '.hb.js', $file);

            if (file_exists(_PS_MODULE_DIR_ . $this->name . '/' . $fileHummingbird)) {
                $file = $fileHummingbird;
            }
        }

        $m = [];
        preg_match('/\/([a-z0-9_]+)\./', $file, $m);

        $name = $this->name . '_' . $m[1];
    ...
    protected function registerStylesheet($file = 'views/css/front/module.css', $cssParams = [])
    {
        if ($this->getConfig()->get('THEME') === 'hummingbird') {
            $fileHummingbird = str_replace('.css', '.hb.css', $file);
