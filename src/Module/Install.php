...
class Install
{
    ...
    public function __construct(\GC_Facetedsearch $module, AbstractSettings $settings)
    {
        $this->module = $module;
    ...
    public function installConfig(): bool
    {
        ...
        try {
             ...
             */
            $configurationadapter = $this->module->get('onlineshopmodule.module.facetedsearch.configurationadapter');
        } catch (\Exception $e) {
    ...
    public function uninstallConfig(): bool
    {
        ...
        foreach ($this->settings->getConfig() as $config) {
            ...
            $configurationAdapter->delete($config->getName(), $config->usePrefix());
        }
    ...
    protected function installHook(Hook $hook)
    {
        $this->module->registerHook((string) $hook);

        $controllerWhitelist = $hook->getControllerWhitelist();

        if (empty($controllerWhitelist)) {
            return;
        }

        $allController = \Dispatcher::getControllersPhpselfList(_PS_FRONT_CONTROLLER_DIR_);
        asort($allController);

        $controllerToInsert = array_diff($allController, $controllerWhitelist);

        $idHook = LegacyHook::getIdByName((string) $hook);

        foreach ($controllerToInsert as $excepts) {
            $this->module->registerExceptions($idHook, $excepts);
        }
    }
}
